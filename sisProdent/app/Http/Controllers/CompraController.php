...
class CompraController extends Controller
{
  ...
  public function index(Request $request){
    if($request){
      ...
      ->select('com.id','com.num_lote', 'com.fecha',DB::raw('doc.nombre as nombre_doctor'),DB::raw('doc.apellido as apellido_doctor'), DB::raw('fp.forma as forma_pago'),'com.detalles')
...
public function store(CompraFormRequest $request /*Request $request*/){
  ...
  public function show($id){
    $compra = DB::table('compra as com')
      ...
      ->join('forma_pago as fp', 'com.forma_pago_id','=','fp.id')
      ->join('detalle_compra as dc', 'com.id','=','dc.compra_id')
      ->select('com.id','com.num_lote', 'com.fecha','com.total', DB::raw('doc.nombre as nombre_doctor'),DB::raw('doc.apellido as apellido_doctor'),DB::raw('sum(dc.cantidad*dc.precio) as total'), DB::raw('fp.forma as forma_pago'))
      ->where('com.id','=',$id)->first();
    $detalles = DB::table('detalle_compra as dc')
      ->join('medicamento as med','dc.medicamento_id','=','med.id')
      ->join('proveedor as pro','dc.proveedor_id','=','pro.id')
