...
class PagosController extends Controller
{
  ...
  public function index(Request $request){
    if($request){
      ...
      $pagos=DB::table('cuenta as cu')->join('tratamiento as tra', 'cu.tratamiento_id','=','tra.id')->select('cu.id','cu.saldo','cu.fecha','tra.saldo_actual','tra.nombre')->where('cu.fecha','LIKE','%'.$query.'%')->orderBy('cu.id','asc')->paginate(7);
