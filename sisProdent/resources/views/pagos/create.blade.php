...
@section('contenido')
	<div class="row">
        ...
        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
				...
				class="form-group">         <label for="saldo">Cantidad a
				Abonar</label>         <input type="number" name="saldo"
				required value="{{old('saldo')}}" class="form-control"
				step="0.01" title="Currency" pattern="^\d+(?:\.\d{1,2})?$" onb
				lur="this.parentNode.parentNode.style.backgroundColor=/^\d+(?:
				\.\d{1,2})?$/.test(this.value)?'inherit':'#CFD8DC'" min="0">
				</div> </div>
